...
//    }
    ...
    @Test
    public void testGetPreviousMessages() throws Exception {
        Message message = new Message();
        message.setContent("Hello World");

        when(groupService.getGroupMessages(anyLong())).thenReturn(Collections.singletonList(message));

        mockMvc.perform(get("/group/1/messages"))
                .andExpect(status().isOk())
